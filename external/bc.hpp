 ...
 */
inline int direction_dx(Direction direction) {
  return bc_Direction_dx(direction);
}
 ...
 */
inline int direction_dy(Direction direction) {
  return bc_Direction_dy(direction);
}
 ...
 */
inline bool direction_is_diagonal(Direction direction) {
  return bc_Direction_is_diagonal(direction);
}
 ...
 */
inline Direction direction_opposite(Direction direction) {
  return bc_Direction_opposite(direction);
}

/**
 * Returns the direction 45 degrees to the left (counter-clockwise) of this one,
 * or Center if it's Center.
 * @param direction
 * @return the direction 45 degrees to the left
 */
inline Direction direction_rotate_left(Direction direction) {
  return bc_Direction_rotate_left(direction);
}

/**
 * Returns the direction 45 degrees to the right (clockwise) of this one, or
 * Center if it's Center.
 * @param direction
 * @return the direction 45 degrees to the right
 */
inline Direction direction_rotate_right(Direction direction) {
  return bc_Direction_rotate_right(direction);
}
...
class MapLocation {
 public:
   ...
   * @param map_location
   */
  explicit MapLocation(bc_MapLocation* map_location)
  ...
  MapLocation add(Direction direction) const {
    ...
    return MapLocation(m_planet, m_x + direction_dx(direction),
  ...
  MapLocation add_multiple(Direction direction, int multiple) const {
    // Hardcoded to avoid API calls
    return MapLocation(m_planet, m_x + direction_dx(direction) * multiple,
  ...
  unsigned distance_squared_to(const MapLocation& map_location) const {
    // Hardcoded to avoid API calls
    if (m_planet != map_location.get_planet()) return INT_MAX;
...
class Location {
 public:
  ...
  Location(const MapLocation& map_location)
      : m_type{Map}, m_map_location{map_location} {}
  Location(unsigned garrison_id)
      : m_type{Garrison}, m_garrison_id{garrison_id} {}
  ...
  bool is_within_range(unsigned range, Location location) {
    ...
    return get_map_location().is_within_range(range,
...
/*  */
inline bool is_structure(UnitType unit_type) {
  return unit_type == Factory or unit_type == Rocket;
}
/*  */
...
/*  */
inline unsigned unit_type_get_value(UnitType unit_type) {
  return bc_UnitType_value(unit_type);
}
...
#define GET(ret, var)                                  \
  ret G(var)() const {                                 \
    auto ans = static_cast<ret>(F(var)(m_unit.get())); \
    CHECK_ERRORS();                                    \
    return ans;                                        \
  }
#define IS(var)                      \
  bool var() const {                 \
    bool ans = F(var)(m_unit.get()); \
    CHECK_ERRORS();                  \
    return ans;                      \
  }
#define GET_FUNC(ret, var, func)           \
  ret G(var)() const {                     \
    auto ans = func(F(var)(m_unit.get())); \
    CHECK_ERRORS();                        \
    return ans;                            \
  }
  ...
  MapLocation get_map_location() const {
    return get_location().get_map_location();
  }
...
class PlanetMap {
 public:
  ...
  PlanetMap(bc_PlanetMap* planet_map) : m_planet_map{planet_map} {
    ...
    m_initial_units =
        to_vector(bc_PlanetMap_initial_units_get(m_planet_map.get()));
  }
  ...
  /*
  // XXX: Not needed! We only need 2 planets: earth and mars. It's not needed to
  set stuff void set_planet(Planet   planet) { m_planet = planet; } void
  set_height(unsigned height) { m_height = height; } void set_width (unsigned
  width ) { m_width = width; }
  */
  ...
  bool is_passable_terrain_at(const MapLocation& map_location) const {
    ...
    auto ans = bc_PlanetMap_is_passable_terrain_at(m_planet_map.get(),
                                                   map_location.get_bc());
    CHECK_ERRORS();
    return ans;
  }
  ...
  unsigned get_initial_karbonite_at(const MapLocation& map_location) const {
    log_error(m_planet_map, "PlanetMap not loaded!");
    auto ans = bc_PlanetMap_initial_karbonite_at(m_planet_map.get(),
                                                 map_location.get_bc());
    CHECK_ERRORS();
    return ans;
  }
   ...
   * This information is constant, so it's useful to have it since the beginning
   * and never use the API for this.
   *
  ...
  std::vector<std::vector<std::pair<bool, unsigned>>> get_initial_map() const {
    std::vector<std::vector<std::pair<bool, unsigned>>> ans{
        m_height, std::vector<std::pair<bool, unsigned>>{m_width}};
    ...
    for (int i = 0; i < (int)m_height; i++) {
      for (int j = 0; j < (int)m_width; j++) {
        ...
        bool passable = bc_PlanetMap_is_passable_terrain_at(m_planet_map.get(),
                                                            mp.get_bc());
        auto karbonite =
            bc_PlanetMap_initial_karbonite_at(m_planet_map.get(), mp.get_bc());
 ...
 private:
  ...
  // TODO: Use the maps to store the values and never query the same position
  // again
 ...
 public:
  ...
  std::unordered_map<unsigned, AsteroidStrike> get_all_strikes() {
    ...
    for (int i = 0; i < MAX_ROUNDS; ++i) {
      if (has_asteroid_on_round(i)) ans[i] = get_asteroid_on_round(i);
    }
    return ans;
  }
...
class OrbitPattern {
 public:
  OrbitPattern(bc_OrbitPattern* orbit_pattern)
      : m_orbit_pattern{orbit_pattern} {
...
class ResearchInfo {
 public:
  ...
  unsigned cost_of(UnitType branch, unsigned level) const {
    return ::cost_of(branch, level);
  }

  unsigned get_level(UnitType branch) const {
    auto r = bc_ResearchInfo_get_level(m_info.get(), branch);
    CHECK_ERRORS();
    return r;
  }
  std::vector<UnitType> get_queue() const {
    auto r = to_vector(bc_ResearchInfo_queue(m_info.get()));
    CHECK_ERRORS();
    return r;
  }
  bool has_next_in_queue() const {
    auto r = bc_ResearchInfo_has_next_in_queue(m_info.get());
    CHECK_ERRORS();
    return r;
  }
  UnitType next_in_queue() const {
    auto r = bc_ResearchInfo_next_in_queue(m_info.get());
    CHECK_ERRORS();
    return r;
  }
  unsigned rounds_left() const {
    auto r = bc_ResearchInfo_rounds_left(m_info.get());
    CHECK_ERRORS();
    return r;
  }
...
class RocketLanding {
 public:
  RocketLanding(bc_RocketLanding* rocket_landing) {
    ...
    m_destination =
        MapLocation(bc_RocketLanding_destination_get(rocket_landing));
...
class RocketLandingInfo {
 public:
  ...
  std::vector<RocketLanding> get_landings_on_round(unsigned round) {
    return to_vector(
...
// GameController
// IMPORTANT: Don't instantiate twice, might fail creating multiple
// bc_GameController
class GameController {
 public:
  ...
  ~GameController() {
    if (m_gc) delete_bc_GameController(m_gc);
  }
  ...
  std::vector<Unit> get_units() const {
    return to_vector(bc_GameController_units(m_gc));
  }
  std::vector<Unit> get_my_units() const {
    return to_vector(bc_GameController_my_units(m_gc));
  }
  std::vector<Unit> get_units_in_space() const {
    return to_vector(bc_GameController_units_in_space(m_gc));
  }
  ...
  std::vector<MapLocation> get_all_locations_within(
      const MapLocation& map_location, unsigned radius_squared) const {
    return to_vector(bc_GameController_all_locations_within(
        m_gc, map_location.get_bc(), radius_squared));
  }
  ...
  bool can_sense_unit(unsigned id) const {
    return bc_GameController_can_sense_unit(m_gc, id);
  }

  std::vector<Unit> sense_nearby_units(const MapLocation& map_location,
                                       unsigned radius_squared) const {
    return to_vector(bc_GameController_sense_nearby_units(
        m_gc, map_location.get_bc(), radius_squared));
  }

  std::vector<Unit> sense_nearby_units_by_team(const MapLocation& map_location,
                                               unsigned radius_squared,
                                               Team team) const {
    return to_vector(bc_GameController_sense_nearby_units_by_team(
        m_gc, map_location.get_bc(), radius_squared, team));
  }

  std::vector<Unit> sense_nearby_units_by_type(const MapLocation& map_location,
                                               unsigned radius_squared,
                                               UnitType type) const {
    return to_vector(bc_GameController_sense_nearby_units_by_type(
        m_gc, map_location.get_bc(), radius_squared, type));
  }
  ...
  Unit sense_unit_at_location(const MapLocation& map_location) const {
    auto ans =
        bc_GameController_sense_unit_at_location(m_gc, map_location.get_bc());
    CHECK_ERRORS();
    return Unit(ans);
  }

  const AsteroidPattern& get_asteroid_pattern() const {
    return m_asteroid_pattern;
  }
  ...
  bool reset_research() const { return bc_GameController_reset_research(m_gc); }
  ...
  bool can_blueprint(unsigned id, UnitType unit_type,
  ...
  bool can_begin_snipe(unsigned ranger_id,
                       const MapLocation& map_location) const {
    return bc_GameController_can_begin_snipe(m_gc, ranger_id,
                                             map_location.get_bc());
  }
  ...
  bool can_begin_blink(unsigned mage_id,
                       const MapLocation& map_location) const {
  ...
  bool can_launch_rocket(unsigned rocket_id,
                         const MapLocation& map_location) const {
    return bc_GameController_can_launch_rocket(m_gc, rocket_id,
                                               map_location.get_bc());
  }

  void launch_rocket(unsigned rocket_id,
                     const MapLocation& map_location) const {
